...
test_that(".check_environment works", {
    ...
    cor_opt <- getOption("pyspec_copy_on_replace")
    cor_opt_sys <- Sys.getenv("pyspec_copy_on_replace")
    Sys.unsetenv("pyspec_copy_on_replace")
    options(pyspec_copy_on_replace = NULL)
    .check_environment()
    res <- getOption("pyspec_copy_on_replace")
    expect_null(res)
    Sys.setenv(pyspec_copy_on_replace = "false")
    .check_environment()
    res <- getOption("pyspec_copy_on_replace")
    expect_false(res)
    options(pyspec_copy_on_replace = NULL)
    Sys.setenv(pyspec_copy_on_replace = "TRUE")
    .check_environment()
    res <- getOption("pyspec_copy_on_replace")
    expect_true(res)
    options(pyspec_copy_on_replace = NULL)
    ...
    expect_true(res)
    options(pyspec_copy_on_replace = NULL)
    Sys.setenv(pyspec_copy_on_replace = "1")
    .check_environment()
    res <- getOption("pyspec_copy_on_replace")
    expect_true(res)
    options(pyspec_copy_on_replace = FALSE)
    Sys.setenv(pyspec_copy_on_replace = "1")
    .check_environment()
    res <- getOption("pyspec_copy_on_replace")
    expect_false(res)
    ...
    if (length(cor_opt))
        options(pyspec_copy_on_replace = cor_opt)
    if (cor_opt_sys != "")
        Sys.setenv(pyspec_copy_on_replace = cor_opt_sys)
})
