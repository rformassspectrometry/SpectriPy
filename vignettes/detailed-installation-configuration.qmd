...
*reticulate*. Note that the path specified below in the `use_python()` call
might be different depending on the operating system or setup. Eventually refer
to the installation log/output from the previous code block if miniconda was
installed with the `install_miniconda()` function.
