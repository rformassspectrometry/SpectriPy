...
operation system settings, or specifically for R. In R, it is possible to set
the variable using `Sys.setenv()` **before** the *SpectriPy* or *reticulate*
libraries are loaded. For example with:
...
Since *SpectriPy* is currently still in Bioconductor's developmental branch and
needs to be installed through GitHub. For that, the *remotes* package needs to
be installed.
