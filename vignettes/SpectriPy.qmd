...
languages. Specifically, the package translates between R's 
`Spectra` objects (from the `r BiocStyle::Biocpkg("Spectra")` package) and 
Python's `matchms.Spectrum` objects from the 
[*matchms*](https://github.com/matchms/matchms) Python library.


# Installation SpectriPy
...
available. If they are not available, *SpectriPy* will install them using
