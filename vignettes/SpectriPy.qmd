...
@sec-python for more Python configuration options of *SpectriPy*.
...
information). A more elegant way would be to use the `MsBackendPy` backend for
...
Similarly, we can also provide a custom mapping for the `MsBackendPy`:

```{r}
#' R session:

#' Add mapping for additional spectra variables to the `MsBackendPy`
mgf <- Spectra("mgf_p", source = MsBackendPy(), mapping = map)
spectraVariables(mgf)
```

And we have now also access to the additional metadata:

```{r}
#' R session:

#' List the first values for the spectra variable
mgf$compound_name |>
    head()
```
