...
*SpectriPy* will install also all required Python libraries into a new Python
environment *r-spectripy*. By default Python *virtualenv* are used, but also
*miniconda* can used instead. See section @sec-python for options and how to
skip this and use the system Python environment instead.
...
*SpectriPy* will use *virtualenv* to setup and use a package-specific
Python which includes all required Python libraries (i.e. *matchms* version
0.28.2 and *spectrum_utils* version 0.3.2, loaded from *conda-forge*). The
Python setup can also be configured with three package options that can also be
set *via* environment variables. These are:
