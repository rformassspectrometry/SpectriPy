...
can be checked by performing some steps from the package’s vignette, from source
...
`compareSpectra` from Spectra [@rainer_modular_2022], `CosineGreedy` from
matchms [@huber_matchms_2020], `MS2DeepScore` from matchms 
[@huber_ms2deepscore_2021] and `Spec2Vec` from spec2vec [@huber_spec2vec_2021].
...
e.g. to compare SMILES across spectra, such as the `MetadataMatch` from matchms 
and the Tanimoto similarity search with rdkit and fingerprint R packages.
...
Load required R SpectriPy package. If you already have a Python environment 
opened, please restart your Integrated Development Environment and run this
as a first comment, to load the required package reticulate and setup the conda 
environment 'r-spectripy' correctly. Please see the 
[Detailed information on installation and configuration](detailed-installation-configuration.qmd)
for other options.
