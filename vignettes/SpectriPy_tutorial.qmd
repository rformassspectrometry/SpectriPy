...
combining the strengths of Python and R MS libraries. To showcase the strength
...
[@rainer_modular_2022], XX from matchms [@huber_matchms_2020], Tanimoto
score predictions from MS2DeepScore, XX. FINDME!!!
...
- Python: process (min 2 fragm present or matchms cleaning) and filter the data, both the reference database and the experimental data
...
- compare our filtering/... with the rest of metabonaut workkflow (of the significant)
...
Python: calculate pairwise similarity (= CosineGreedy from matchms) between the experimental spectra and reference.
R: translate back to R
R: plot, maybe a graph?


Maybe a second or other use case? Something that would start with a Python MS data and we could use that with the MsBackendPy?
The MsBackendPy class that can be used as a data backend for R’s Spectra allows a very efficient and memory save direct access to MS data in Python by translating the data on-the-fly and only when needed.  => we can maybe time the speed of the first tutorial above with a big dataset (the res from metabonaut), then use the fastest strategy in the tutorial and explain these. So we end up with only 1 tutorial as a vignette with best practice tips.


Third example can be something using the received annotations eg inchikey, eg after some public MS1 DB matching (metabonaut section “MS1-based annotation”, get inchi from massbank). Maybe can match to found from ms2 or something. Then mention same can be done with the SMILES, name, etc using func matchms. => X less useful than the 1st tutorial, less is more. Also would be better to do something with other py package than matchms.
