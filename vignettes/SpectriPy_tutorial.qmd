...
a reference library. During the tutorial, we use different MS2 filtering and
spectral similarity algorithms from both R and Python.

The three different filtering methods which will be demonstrated throughout this
tutorial are: default filtering (`filterMsLevel()`) from Spectra
[@rainer_modular_2022], default filtering (`default_filters`,
`normalize_intensities`) from *matchms( [@huber_matchms_2020], and using a pre-
and post-processing pipeline from *matchms*'s `msfilters`.

The four MS2 spectral similarity algorithms which are demonstrated below are:
`compareSpectra()` with the default *normalized dot product* similarity score
from *Spectra* [@rainer_modular_2022], `CosineGreedy` from *matchms*
[@huber_matchms_2020], `MS2DeepScore` from *matchms* [@huber_ms2deepscore_2021]
and `Spec2Vec` from *spec2vec* [@huber_spec2vec_2021].

The spectral reference library used for the annotation of the unknown features
in this tutorial originates from an *in-house* reference library (provided in
MGF format), but can easily be replaced by other (public) MS2 spectral libraries
such as MGF file from the GNPS repository or even with a pre-trained model
containing spectra from multiple public sources. For a demonstration of using
pre-trained models, see the *MS2DeepScore* section. We also showcase how to make
your own model with using an *in-house* reference library in the Spec2Vec
section.

The spectral filtering and similarity algorithms used in this
tutorial, can easily be extended to other R/python spectral filtering and
silimilatity algorithms. Including similarity algorithms comparing metadata,
e.g. to compare SMILES across spectra, such as the `MetadataMatch` from
*matchms* and the Tanimoto similarity search with rdkit and fingerprint R
packages.
...
+Python: calculate pairwise spectral similarity (= CosineGreedy from matchms) between the experimental spectra and reference.
-py: others cals...!!! TODO
-MS2DeepScore
    ...
    - ok with torch, msdeepscore dependencies add to spectripy? HOW? jo?
    - codechunks from Matthias OK, nok in here because needs own py-chunks...
    - change object to use,
-spec2vec
    steps before mrd TODO
    + query spectra() -> mgf (or how describe/logical?) ok
    - spec2vec dependencies add to spectripy? HOW? jo?
    + codechunks from Marilyn OK
-more COSINEs from matchms?? cross-language-ms-analysis.qmd for other examples. TODO
-py bugs in retic!!!! see TODO, ok if render but not 'handy' for py users
+py/r: table with the top1 found names of molec for the query???
-R: translate back to R !!! TODO how?
+R: plot
-TODO change so now we have 4 comps... maybe a heatmap with 1st spectrum (caffeine)
and the scores of each??
-at end (how?), put data in vignette IN package, now via explicit path pc... TODO, ask Jo how?
- WE CAN easily add 2 SMILES search examples too, but not really our usecase
(we start with unknown...), maybe only in the paper. since have the 2 code
chuncks, just not fit here.

change to 2 vignettes at end, select only needed FINDME and describe. TODO:

- [] this vignette with test, test 3 different filtering/processing
  (of both query/ref) + 4 similariy
  scores, plus describe this can be expanded to other algorithms and even to
  comarison with other metadate. eg SMILES somparison with matchms
  (see cross-...) abd with the Tomo... from MS2Deep/matchms.
- [] metabonaut with load her object, only 1 ex filtering and 1 matchms algo.
  end is the table eval and the give info back into the spectra object.

[jo: would prefer not adding/depending SpectriPy on spec2vec and MS2deepscore;
could we move these to metabonaut instead? there we could manually install the
packages/dependencies in the docker image. Generally, would avoid having
duplicated examples. So, either having this here, or in metabonaut, not in
both? Another reason is maintenance, if it's in SpectriPy we have to ensure it
does not fail - otherwise we have issue with Bioconductor.]
=> TBD
...
filtering in Spectra (processing method 1).
...
[jo: do we need to import from mgf? or should we use the MassBank database as in
the end-to-end? for mgf, we would need to download e.g. GNPS and then load
it. For MassBank we could load the data in R as a `Spectra` and translate that
to Python.] => end-to-end: ancy way. this is to really understand it, so much 
easier because can actually see what is a spectral libr.
GNPS Library List
GNPS Library 	Library Type 	Browse at GNPS 	MGF Download 	MSP Download 	JSON Download
GNPS-LIBRARY 	GNPS 	Browse 	Download 	Download 	Download
https://external.gnps2.org/gnpslibrary

## GNPS spectral library

The MS2 spectral reference library in this tutorial originates from the GNPS
repository, and is provided as an MGF formatted file.  To load other file
formats, see
...
We import a test data file in MGF format using the Python *matchms*
...
r.ms2_ctr_fts_py[0].get('charge') #NOK!!
r.ms2_ctr_fts_py[0].metadata.keys()
...
library, we apply some filtering from matchms (processing method 2).  Default
filtering from matchms was performed to standardize ion mode, correct charge and
...
We calculate the pairwise spectral similarity between the query spectra and the reference
library spectra using different approaches, using spectral objects (Spectra,
Spectrum) accross both languages and using algoritms from multiple libraries.


## CosineGreedy from matchms

next, we calculate the pairwise spectral similarity between the query spectra and the
...
other similarity score methods.
...
on how to generate the mirror plots.


## compareSpectra from Spectra

Here, we calculate the pairwise spectral similarity between the query spectra
and the reference library spectra using R's Spectra.
...
#' Calculate the pairwise spectral similarity between spectra
...
Here, we calculate the pairwise spectral similarity between the query spectra
and the pre-trained model from MS2DeepScore. This model called
"ms2deepscore_model.pt" was trained on > 500,000 MS/MS combined spectra from
GNPS, Mona, MassBank and MSnLib, by the developers of MS2DeepScore. See
[here](https://github.com/matchms/ms2deepscore) for more information.
No MS2 spectral filtering is performed in this example.

[jo: we would need to get this model.pt somehow downloaded automatically - is it
maybe on Zenodo? or github?]
Available on Zenodo: https://zenodo.org/records/13897744
and also on the https://github.com/matchms/ms2deepscore github
...
#' Define file path to the pre-trained model from MS2DeepScore
model_file_name = "/home/mdegraeve/Documents/Files/Work_Eurac/Projects/SpectriPy/inst/extdata/pt/ms2deepscore_model.pt"

#' Load in the ms2deepscore model
model = load_model(model_file_name)
```

Using the trained model, binned spectrums will be converted into spectrum
...
similarity_score = MS2DeepScore(model = model)
#scores = calculate_scores(references = clean_mgf_py,
#                          queries = r.ms2_ctr_fts_py,
#                          similarity_function = similarity_score)
#scores
...
TODO, FINDME, NOK!!! should work, see https://github.com/matchms/ms2deepscore/blob/main/ms2deepscore/MS2DeepScore.py

```{python}
#' Python session:

#TypeError: Expected float or int, got None, for precursor_mz
#=> precursor_mz also not found, while there:
r.ms2_ctr_fts_py[0].get('precursor_mz')
clean_mgf_py[0].get('precursor_mz')

#=> pos/neg but 'CHARGE=1+' everywhere present! works sep ref_path too, just not both!!
#ValueError: Feature should be ['positive'] or ['negative'], not None
clean_mgf_py[0].get('charge')
r.ms2_ctr_fts_py[0].get('charge') #NOK!!
#also think I need this in the mgf file while load:
# IONMODE=positive
...
#' Convert to array and transpose
sim_ms2deepscore = scores.to_array()
sim_ms2deepscore = sim_ms2deepscore.T

#' Contains 1 row for each spectrum in query
sim_ms2deepscore.shape
```
...
Here, we calculate the pairwise spectral similarity between the query spectra
and the reference library spectra using Pythons's spec2vec.

We also apply a stricter filtering processing from matchms (processing method
3). This time we will use a more advance filtering, in a pre- and
post-processing pipeline uing `msfilters` from `matchms`.  The filtering will be
performed on both the reference and query spectra, and for this reason we will
have a shorter results table.
