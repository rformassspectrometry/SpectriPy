...
"SpectriPy.Rmd":
 ...
 but can(not) run in Quarto using `reticulate` in the background:
  ...
  - [ ] IntersectMz using IntersectMzParam
  - [ ] MetadataMatch using MetadataMatchParam
  - [ ] ParentMassMatch using ParentMassMatchParam
  - [ ] PrecursorMzMatch using PrecursorMzMatchParam
...
print(spectrum)
print(spectrum.peaks.mz[0])
print(spectrum.get('charge'))
print(spectrum.get('inchi'))
...
system.time(
    all_ms2_py <- rspec_to_pyspec(all_ms2, mapping = c(smiles = 'smiles', inchi = 'inchi'))
)
all_ms2_py
```
